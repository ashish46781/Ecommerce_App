...
Stage 2 - Lesson 3: add the Category-to-Product database and ORM relationship.
Implemented and verified. Teaching covers one-to-many and many-to-one relationships,
foreign keys, nullable schema evolution, deletion rules, relationship navigation,
foreign-key indexing, joins, and reviewing an autogenerated Alembic revision.
Next lesson: Stage 2 - Lesson 4: enforce Product price and stock rules in PostgreSQL.
...
- [ ] Stage 2 - Lesson 4: add database `CHECK` constraints for positive Product price
  and non-negative stock.
...
- Product price and stock do not yet have database `CHECK` constraints; add and teach
  those in Stage 2 - Lesson 4. Timestamps are also deferred.
- Category is related to Product in the database and ORM but is not exposed through the
  API yet. Existing products have null `category_id`; add Category API operations and
  product assignment before deciding whether the relationship can become required.
  Category-name uniqueness remains case-sensitive.
