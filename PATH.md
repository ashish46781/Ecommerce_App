...
- [x] Stage 2 - Lesson 3 implementation: nullable `products.category_id`, a named
  foreign key to `categories.id` with restricted parent deletion, an index on the
  referencing column, and bidirectional SQLAlchemy relationships.
- [x] Stage 2 - Lesson 3 verification: reviewed offline migration SQL, successful live
  upgrade/downgrade/re-upgrade, exact schema inspection, relationship assignment and
  navigation in both directions, join behavior, rejection of invalid category IDs and
  referenced-category deletion, rollback cleanup, no schema drift, and unchanged API.
- [x] Stage 2 - Lesson 4 implementation: named database checks require `price > 0` and
  `stock >= 0`; Alembic's built-in named-check plugin is enabled for future additions
  and removals, and a reviewed migration adds both constraints.
- [x] Stage 2 - Lesson 4 verification: existing-data preflight, reviewed offline SQL,
...
- Stage 2 - Lesson 3: one-to-many versus many-to-one, parent and child tables,
  referencing and referenced columns, database foreign keys versus ORM relationships,
  nullable foreign keys for safe evolution, referential integrity, `ON DELETE RESTRICT`,
  `back_populates`, relationship synchronization and navigation, lazy loading, joins,
  foreign-key indexes, passive deletes, and reviewing autogenerated migrations.
- Stage 2 - Lesson 4: defense in depth, API validation versus database enforcement,
...
workflow. Alembic's built-in named-check plugin is enabled because every project check
constraint is explicitly named; it detects additions and removals by name but does not
compare changed expressions. Future autogenerated revisions must still be reviewed.
...
currently case-sensitive. The relationship was added only after Alembic could safely
alter the existing `products` table.

### Start the Category-to-Product relationship as optional

Place nullable `category_id` on Product as the many-to-one side and reference
`categories.id` with the named foreign key `fk_products_category_id_categories`.
Expose `Product.category` and `Category.products` as matching ORM relationships through
`back_populates`. Keep the column nullable because products already exist and the API
does not yet accept categories; inventing category assignments during a schema change
would corrupt business meaning. Use `ON DELETE RESTRICT` with passive ORM deletes so a
referenced Category cannot silently orphan or delete Products. Index `category_id`
because PostgreSQL does not automatically index a foreign-key column and category-based
product queries are a realistic access pattern.

### Enforce Product value rules at two boundaries

Keep Pydantic's positive-price and non-negative-stock validation for immediate, clear
API errors. Also define named PostgreSQL checks `ck_products_price_positive` and
`ck_products_stock_non_negative` so direct SQL, scripts, future workers, and application
bugs cannot store invalid values. The database is the final authority for persisted
